defmodule SymphonyElixir.PilotProjection do
  ...
  defp load_dispatch(path, row) do
    with {:ok,
      ...
      {:ok,
       %__MODULE__{
         task: %{id: task_id, identifier: identifier, title: title, objective: objective, state: state, branch: branch, current_head: current_head, base_sha: base_sha},
         lifecycle_id: lifecycle_id,
         working_round_id: round_id,
         planning_attempt_id: attempt_id,
         dispatch_id: dispatch_id,
  ...
  defp decode_scope_list(value) when is_binary(value) do
    case Jason.decode(value) do
      ...
      _ ->
        {:error, :invalid_capability_scope}
    end
  end
  ...
  defp read_json_file(path) do
    case File.read(path) do
      ...
      {:error, reason} ->
        {:error, {:dispatch_file_unreadable, reason}}
    end
  end
    ...
    case Sqlite3.open(path, mode: :readonly) do
      {:ok, connection} ->
        case configure_readonly(connection) do
          ...
          {:error, reason} ->
            _ = Sqlite3.close(connection)
            {:error, {:pilot_database_readonly_setup_failed, reason}}
        end

      {:error, reason} ->
        {:error, {:pilot_database_open_failed, reason}}
    end
  end
