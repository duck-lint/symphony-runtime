defmodule SymphonyElixir.AgentRunner do
  ...
  defp prepare_execution(%PilotProjection{} = dispatch, workspace) do
    if dispatch.role not in @roles do
      ...
      {:ok, %{dispatch | result_root: dispatch.result_path,
        write_roots: Enum.map(dispatch.grant.write_scopes, &Path.join(workspace, &1))}}
    end
  end
  ...
  defp write_launch_evidence(execution, session, workspace) do
    ...
    with {:ok, head} <- git_head(workspace),
      ...
      evidence = %{
        "phase" => "started", "runtime_execution_id" => runtime_execution_id(execution),
        "task_id" => execution.task.id, "lifecycle_id" => execution.lifecycle_id,
        "working_round_id" => execution.working_round_id,
        "planning_attempt_id" => execution.planning_attempt_id,
        "dispatch_id" => execution.dispatch_id, "observed_role" => execution.role,
        "status" => "running", "started_at" => started_at,
        "starting_head" => execution.expected_starting_head, "head_sha" => head,
        "changed_paths" => paths, "dirty" => paths != [],
  ...
  defp notify_launch(recipient, execution, session) do
    if is_pid(recipient) do
      send(recipient, {:runtime_execution_update, execution.dispatch_id,
        %{event: :execution_started, runtime_execution_id: runtime_execution_id(execution),
  ...
  defp finish_execution(session, execution, workspace, result) do
    ...
    try do
      ...
      with {:ok, head} <- git_head(workspace),
           {:ok, paths} <- changed_paths(workspace) do
        evidence = %{
          "phase" => "terminated", "runtime_execution_id" => runtime_execution_id(execution),
          "task_id" => execution.task.id, "lifecycle_id" => execution.lifecycle_id,
          "working_round_id" => execution.working_round_id,
          "planning_attempt_id" => execution.planning_attempt_id,
          "dispatch_id" => execution.dispatch_id, "observed_role" => execution.role,
          "status" => status, "started_at" => launch_started_at(execution),
  ...
  defp changed_paths(workspace) do
    case System.cmd("git", ["status", "--porcelain=v1", "-z", "--untracked-files=all"], cd: workspace, stderr_to_stdout: true) do
      ...
      {_output, status} -> {:error, {:git_status_failed, status}}
    end
  end
