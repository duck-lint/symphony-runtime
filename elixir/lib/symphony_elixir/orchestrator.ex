defmodule SymphonyElixir.Orchestrator do
  ...
  defmodule State do
    @moduledoc false
    defstruct [:poll_interval_ms, :max_concurrent_agents, :tick_ref, :task_supervisor,
      running: %{}, completed: MapSet.new(), last_error: nil,
  ...
  def init(opts) do
    ...
    state = %State{poll_interval_ms: settings.polling.interval_ms,
    ...
    case PilotProjection.next_dispatch(Config.settings!()) do
      ...
      {:ok, %PilotProjection{dispatch_id: dispatch_id} = dispatch} ->
        if MapSet.member?(state.completed, dispatch_id) or Map.has_key?(state.running, dispatch_id) do
          ...
          entry = %{dispatch: dispatch, ref: task.ref, pid: task.pid, started_at: DateTime.utc_now(),
            last_event: nil, last_message: nil, session_id: nil, token_usage: %{}}
  ...
  defp snapshot_payload(state) do
    executions = Enum.map(state.running, fn {dispatch_id, entry} ->
      dispatch = entry.dispatch
      %{task_id: dispatch.task.id, identifier: dispatch.task.identifier, lifecycle_id: dispatch.lifecycle_id,
        working_round_id: dispatch.working_round_id, planning_attempt_id: dispatch.planning_attempt_id,
        expected_role: dispatch.role, dispatch_id: dispatch_id, runtime_execution_id: entry[:runtime_execution_id],
        workspace_path: nil, process_id: entry[:process_id], session_id: entry.session_id,
        started_at: entry.started_at, last_event: entry.last_event, last_message: entry.last_message,
    ...
    %{running: running, authorized_dispatches: authorized_dispatches,
      completed_dispatches: MapSet.to_list(state.completed),
      codex_totals: state.codex_totals, rate_limits: state.rate_limits,
  ...
  defp update_entry(message) when is_map(message) do
    %{last_event: Map.get(message, :event), last_message: inspect(Map.get(message, :message, message), limit: 10),
      session_id: Map.get(message, :session_id), process_id: Map.get(message, :codex_app_server_pid),
