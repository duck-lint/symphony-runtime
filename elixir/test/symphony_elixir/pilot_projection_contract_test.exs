defmodule SymphonyElixir.PilotProjectionContractTest do
  ...
  test "accepts only an exact Pilot dispatch packet and grant" do
    ...
    packet = %{
      ...
      "working_round_id" => row.working_round_id,
      "planning_attempt_id" => row.planning_attempt_id,
      "dispatch_id" => row.dispatch_id,
      "role" => row.role,
      "expected_starting_head" => row.expected_starting_head,
      "capability_grant" => %{
        "id" => row.grant_id,
        "task_id" => row.task_id,
        "dispatch_id" => row.dispatch_id,
        ...
        "read_scopes" => row.read_scopes,
        "write_scopes" => row.write_scopes
      }
    }
  ...
  test "Pilot-selected handoffs are rendered, while Runtime supplies no context" do
    dispatch = %PilotProjection{
      task: %{id: uuid(), identifier: "T-000001", title: "task", objective: "objective"},
      lifecycle_id: uuid(),
      working_round_id: uuid(),
      planning_attempt_id: uuid(),
      dispatch_id: uuid(),
      ...
      namespace: "/tmp/dispatch",
      result_path: "/tmp/dispatch/outbox/result.json",
      handoff_inputs: [%{"kind" => "PM_HANDOFF", "source_role" => "PROJECT-MANAGER", "source_dispatch_id" => uuid(), "input" => %{"summary" => "bounded request"}}]
    }
